...
const MOCK_FACULTY_DATA = [
    {
        ...
        experience: '15 Years',
        coursesAssigned: 3,
        image: 'https://via.placeholder.com/150/007bff/ffffff?text=AK', // Placeholder image
        status: 'Active',
    },
    {
        _id: 'f102',
        ...
        experience: '10 Years',
        coursesAssigned: 1,
        image: 'https://via.placeholder.com/150/28a745/ffffff?text=BA', // Placeholder image
        status: 'On Leave',
    },
    {
        _id: 'f103',
        ...
        experience: '3 Years',
        coursesAssigned: 4,
        image: 'https://via.placeholder.com/150/ffc107/333333?text=SB', // Placeholder image
        status: 'Active',
    },
    {
        _id: 'f104',
        ...
        experience: '7 Years',
        coursesAssigned: 2,
        image: 'https://via.placeholder.com/150/dc3545/ffffff?text=OT', // Placeholder image
        status: 'Inactive',
    },
];

const FacultyList = () => {
    const navigate = useNavigate(); // Kept for navigation, but actual routing won't work in a snippet
    ...
    // --- START: Mock Data Fetching ---
    useEffect(() => {
        const fetchFaculties = () => {
            // Simulate API delay
            setTimeout(() => {
                setFaculty(MOCK_FACULTY_DATA);
                setLoading(false);
            }, 1000);
        }
        fetchFaculties();
    ...
    const handleDelete = (id) => {
        ...
        const confirmDelete = window.confirm("Are you sure you want to delete this faculty member? (SIMULATED)");
        if (!confirmDelete) return

        const member = faculty.find(f => f._id === id);

        // Simulate successful deletion
        console.log(`SIMULATION: Deleting faculty member with ID: ${id}`);
        // Update local state directly
        setFaculty((prev) => prev.filter((f) => f._id !== id));
        alert(`SUCCESS: ${member?.name || "Faculty member"} was simulated deleted successfully!`);
    }
    // --- END: Mock Data Fetching ---

    const departments = ['all', ...new Set(faculty.map(f => f.department))];
    const designations = ['all', 'Professor', 'Associate Professor', 'Assistant Professor', 'Lecturer', 'Visiting Faculty'];
    ...
    const filteredFaculty = faculty.filter(member => {
        ...
        const matchesSearch = memberName.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            member.employeeID?.toLowerCase().includes(searchTerm.toLowerCase()); // Added Employee ID search

        const matchesDepartment = filterDepartment === 'all' || memberDepartment === filterDepartment;
        const matchesDesignation = filterDesignation === 'all' || memberDesignation === filterDesignation;
    ...
    if (loading) {
        return (
            <div className="faculty-list-container text-center mt-5">
                <h3><i className="fas fa-spinner fa-spin me-2"></i>Loading Faculty Data...</h3>
            </div>
        );
    }
